...
func load_timestamps(selector):
	...
	if len(timestamp_list)-1 >= selector:
...
func load_space():
	...
	var nodes: Array = thoughtbubble_store.load_thoughts(["Text-Thought"])
	nodes.remove_at(nodes.find(get_parent().get_name()))
	for node in nodes:
		print_debug(node)
		load_thought(node)
	emit_signal("load_links")
...
func save():
	...
	save_timestamp(str(timestamp))
	var currentTimestamp = null
	if get_parent().current_timestamp != null && get_parent().current_timestamp != "":
	...
	update_timestamp_list()
	get_parent().timestamp_selector = len(timestamp_list)-1
	load_timestamps(len(timestamp_list)-1)
